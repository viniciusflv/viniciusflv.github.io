...
Para executar uma imagem podemos acessar o [Docker Hub](https://hub.docker.com/ 'Docker Hub'), que é um repositório de imagens de código aberto publicadas, e procurar uma imagem. Vou começar escolhendo a imagem oficial do [Ubuntu](https://hub.docker.com/_/ubuntu 'Imagem Ubuntu').

```shell
docker run ubuntu
```

Sempre que executar uma imagem o seguinte fluxo irá acontecer:

<Mermaid
  chart={`
    flowchart LR
      local{Tem imagem local?}
      exe[Executa a imagem]
...
Podemos executar um comando da imagem declarando ele na frente do id da imagem:
...
E em outro terminal e execute o comando:

```shell
docker ps
```

### Deletando images e containers
